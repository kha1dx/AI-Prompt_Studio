...
function CallbackErrorContent() {
  ...
  const getErrorMessage = () => {
    ...
    if (
      errorState.error &&
      ERROR_MESSAGES[errorState.error as keyof typeof ERROR_MESSAGES]
    ) {
      return ERROR_MESSAGES[errorState.error as keyof typeof ERROR_MESSAGES];
    }
    return (
      errorState.error || "An unknown error occurred during authentication"
    );
  };
          ...
          {errorState.error === "exchange_failed" && (
            <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-md p-3 mb-4">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                This is typically a PKCE flow issue. Check browser console for
                detailed logs.
              </p>
            </div>
          )}
